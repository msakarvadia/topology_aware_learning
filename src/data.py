...
T = t.TypeVar("T")
"""
 type variable used throughout Flight.
"""
...
def brightness(x, severity=1):
    ...
    return torch.from_numpy(np.clip(x, 0, 1)).permute(2, 0, 1)  # * 255


def pixelate(x, severity=1):
    # x shape: (3, 32, 32) -> transpose to (32, 32, 3) for OpenCV
...
def elastic_transform(x, severity=1):
    ...
    x = map_coordinates(image, indices, order=1, mode="reflect").reshape(shape)
    return torch.from_numpy(x.transpose(2, 0, 1))
...
def contrast(x, severity=1):
    ...
    x = (x - means) * c + means  # np.clip((x - means) * c + means, 0, 1) * 255
    ...
    if "tiny_mem" in data_name:
        ...
        for idx, (img, label) in enumerate(backdoor_data):
            ...
            if "cifar" in data_name:
                ...
                if ood_type == "defocus":
                    img = defocus_blur(img, blur_level)
                if ood_type == "contrast":
                    img = contrast(img, blur_level)
                if ood_type == "elastic":
                    img = elastic_transform(img, blur_level)
                if ood_type == "pixelate":
                    img = pixelate(img, blur_level)
                if ood_type == "brightness":
                    img = brightness(img, blur_level)
                if ood_type == "zoom_blur":
                    img = zoom_blur(img, blur_level)
                if not many_to_one:
                    label = (label + 1) % num_labels
